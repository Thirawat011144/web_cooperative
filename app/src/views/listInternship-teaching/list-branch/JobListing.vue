    ...
    <div class="bg">
      <div class="container">
        <div class="header">
          ...
          <router-link to="/internshiped">
            <button class="post-job-button">ย้อนกลับ</button>
          </router-link>
        </div>
        <div class="content">
          <div class="job-listing" v-for="job in paginatedJobs" :key="job.id">
            ...
            <div class="job-details">
              <p>
                <span class="text-bold">ที่อยู่:</span> {{ job.companyAddress }}
              </p>
              <p>
                <span class="text-bold">โทรศัพท์:</span> {{ job.companyPhone }}
              </p>
            </div>
          </div>
        </div>
...
const props = defineProps({
  branchName: String, // ชื่อสาขาที่รับมาจาก dynamic route
});

// กำหนดชื่อสาขาให้ตรงกับ title
...
const fetchJobs = async () => {
  try {
    ...
    jobs.value = response.data
      .filter((job) => {
        if (
          job.userDetails.branch === title && // กรองข้อมูลด้วย branchName
...
.post-job-button {
  background-color: rgb(196, 49, 183);
  color: white;
  padding: 10px 20px;
  border: none;
  border-radius: 5px;
