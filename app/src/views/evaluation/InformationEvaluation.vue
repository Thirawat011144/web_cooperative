<template>
  <div class="flex-grow-1 p-3">
    ...
    <div class="card" v-if="dataResult">
      <div class="card-body">
        <h5 class="card-title">
          <span>ชื่อ:</span> {{ dataResult.firstName }} <span>นามสกุล:</span>
          {{ dataResult.lastName }}
        </h5>
        <p class="card-text">
  ...
  if (token && userData && userData.id) {
    try {
      const response = await axios.get(
        `${config.api_path}/evaluation/${userData.id}`,
        {
          headers: {
      ...
      if (response.data) {
        ...
        localStorage.setItem(
          config.currentStudyField,
          response.data.currentStudyField
        );
      }
    } catch (error) {
...
const updateCurrentStudyField = () => {
  console.log(
    "Current study field updated to:",
    dataResult.value.currentStudyField
  );
};
...
const saveChanges = async () => {
  ...
  if (result.isConfirmed) {
    try {
      const token = localStorage.getItem(config.token_name);
      await axios.put(
        `${config.api_path}/evaluation/${dataResult.value.id}`,
