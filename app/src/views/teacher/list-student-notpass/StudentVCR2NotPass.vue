...
const fetchData = async () => {
    try {
        ...
        users.value = response.data.filter(user => (user.status === "ไม่อนุมัติ" || user.status === "ไม่ผ่าน") && user.year === "ปวช 3" && user.branch === branch);
    } catch (error) {
        Swal.fire({
            title: "error",
    ...
    <section class="content mt-4">
        <div class="card">
            <div class="card-header">
                <div class="card-title mb-2">ข้อมูลนักศึกษาชั้นประกาศนียบัตรวิชาชีพ ชั้นปีที่ 3 (ไม่ผ่าน)
                    <div>
