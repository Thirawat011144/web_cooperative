...
const fetchData = async () => {
    try {
        ...
        users.value = userResponse.data.filter(user => user.status === changeStatus.value && user.year === "ป.ตรี ปีที่ 4" && user.branch === branch);
    ...
    <section class="content mt-4">
        <div class="card">
            <div class="card-header">
                <div class="card-title mb-2">ข้อมูลนักศึกษาชั้นปริญญาตรีชั้นปีที่ 4 (ไม่ผ่าน)
                    <div>
                        ...
                        <button class="btn btn-info m-1" v-if="changeStatus === 'ไม่ผ่าน'"
                            @click="downloadExcel('student', sortedUsers)">ดาวน์โหลด[การประเมิน]</button>
                        <button class="btn btn-info m-1"
                            @click="downloadExcelHight('stucent', sortedUsers)">ดาวน์โหลด[ข้อมูลส่วนตัว]</button>
                    </div>
