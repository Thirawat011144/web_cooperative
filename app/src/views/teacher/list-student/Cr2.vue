    ...
    <div class="card">
      <div class="card-header">
        <div class="card-title mb-2">
          ข้อมูลนักศึกษาชั้นประกาศนียบัตรวิชาชีพ ชั้นปีที่ 3 <br>
          <div class="btn-group">
        ...
        <tbody>
          <tr v-for="(user, index) in sortedUsers" :key="user.id">
            ...
            <td v-if="selectedAction === 'เข้ารับการฝึก'">
              <button
                :class="(user.evaluationUniversityDetails.length > 0 || user.evaluationDetails.length > 0) ? 'btn btn-secondary' : 'btn btn-success'"
                @click="handleEvaluation(user.id)"
                :disabled="(user.evaluationUniversityDetails.length > 0 || user.evaluationDetails.length > 0)">
                {{ (user.evaluationUniversityDetails.length > 0 || user.evaluationDetails.length > 0) ? 'ประเมินแล้ว' :
                'ประเมิน' }}
              </button>
            </td>
          </tr>
...
const fetchData = async (users, status = null) => {
  try {
    ...
    let filteredUsers = response.data.filter((user) =>
      user.year === "ปวส 2" && branch.some(b => b.name === user.branch)  // เปลี่ยน "ปวช 3" เป็น "ปวส 2"
    );
